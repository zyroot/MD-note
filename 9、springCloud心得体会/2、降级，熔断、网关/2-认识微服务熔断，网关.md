...
@SpringCloudApplication 包含了一下注解

> @SpringBootApplication
>
> @EnableDiscoveryClient     客户端发现服务
>
...
public class UserDao {
    ...
    private static final Logger logger = LoggerFactory.getLogger(UserDao.class);
  	...
  	//第二步：标记此方法做降级处理，配置超时时长
    @HystrixCommand(
    	@HystrixProperties(name = "execution.isolation.thread.timeoutInMilliseconds"
                           ,value="2000")
    )
    public User queryUserById(Long id){
        long begin = System.currentTimeMillis();
...
而==Hystix的超时时间默认也是1000ms==，因此重试机制没有被触发，而是先触发了熔断。
...
- Open：打开状态（断路器打开），所有请求都会被降级。Hystix会对请求情况计数，当一定时间内失败请求百分比达到阈值，则出发熔断，断路器会完全关闭。默认失败比例的阈值是50%，请求次数最少不低于20次。
- Half Open：半开状态，Closed状态不是永远的，关闭后会进入休眠时间（默认5秒）。随后断路器会自动进入半开状态。此时会释放部分请求通过，若这些请求都是健康的，则会完全打开断路器，否则继续保持关闭，再次进行休眠计时。
...
@GetMapping
@HystrixCommand(
  @HystrixProperties(name = "circuitBreaker.requestVolumeThreshold "
                     ,value="10"
...
public String queryById(@PathVarible("id")Long id){
  if(id == 1){
...
如果参数是id为1，一定失败，其他情况都成功。

我们准备两个请求窗口：

一个请求：http://localhost:8080/comsumer/1,注定失败

一个请求：http://localhost:8080/comsumer/2,注定成功

当我们疯狂访问id为1的请求时（超过20次），就回触发熔断。断路器端口，一起请求都会被降级处理。此时你访问id为2的请求，会发现返回的也是失败，而且失败时间很短，只有20毫秒左右；

不过，默认的熔断触发要求较高，休眠时间窗较短，为了测试方便，我们可以通过配置修改熔断策略：

> 1  circuitBreaker.requestVolumeThreshold = 10
>
...
- 你会发现RestTemplate的注册被我删除了。==Feign已经集成了Ribbon自动实现负载均衡==，因此我们不需要自己定义RestTemplate了
